...
export default function ManiadePetLanding() {
  ...
  return (
    <div className="min-h-screen font-sans bg-white" style={{ fontFamily: "'Nunito', 'Poppins', system-ui, sans-serif" }}>
      ...
      {/* ── SERVIÇOS (AJUSTADO PARA 3 COLUNAS) ────────────────────────────────── */}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-gray-500 text-lg max-w-xl mx-auto">
              Clínica veterinária e pet shop completos para cuidar da saúde do seu melhor amigo.
            </p>
          </div>

          {/* Mudança crucial aqui: lg:grid-cols-3 para centralizar os 3 itens */}
                ...
                <ul className="space-y-2 mt-auto pt-4">
                  {s.items.map((item, j) => (
                    <li key={j} className="flex items-center gap-2 text-sm text-gray-700">
                      <span className={`w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center text-white text-[10px] ${s.badge.replace("text-", "bg-").split(" ")[0]}`}>✓</span>
                      {item}
                    </li>
                  ))}
                </ul>
                <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer"
                  className="mt-4 flex items-center justify-center gap-2 bg-white border-2 border-current text-teal-700 font-bold text-sm py-3 rounded-xl hover:bg-teal-50 transition-all">
